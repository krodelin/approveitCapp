...
app (projectName, function(task)
{
    ...
    task.setBuildIntermediatesPath(FILE.join("Build", "approveitCapp.build", configuration));
    task.setBuildPath(FILE.join("Build", configuration));

    task.setProductName("approveitCapp");
    task.setIdentifier("com.krodelin.approveitCapp");
    task.setVersion("1.0");
...
task ("desktop", ["release"], function()
{
    ...
    require("cappuccino/nativehost").buildNativeHost(FILE.join("Build", "Release", projectName), FILE.join("Build", "Desktop", projectName, "approveitCapp.app"));
    printResults("Desktop")
});

task ("run-desktop", ["desktop"], function()
{
    OS.system([FILE.join("Build", "Desktop", projectName, "approveitCapp.app", "Contents", "MacOS", "NativeHost"), "-i"]);
});
